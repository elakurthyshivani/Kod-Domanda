...
$q="SELECT * FROM Contacted ORDER BY IsRead, MID DESC";
...
<?php for($i=1; $i<=$nobc; $i++): ?>	
	...
	<?php for($j=($i-1)*10; $j<$tn && $j<$i*10; $j++): ?>
		<div class="group">
    		...
    		<div class="email"><?php echo $c[$j][2]; ?></div>
    		<div class="subject"> <?php echo $c[$j][3]; ?></div>
    		<div class="message"><?php echo $c[$j][4]; ?></div>
    		<button onclick="toggleValues(this, <?php echo $c[$j][0]; ?>)"><?php 
    		     echo ($c[$j][5]==0?"Unread":"Read"); ?></button>
		</div>
	<?php endfor; ?>
	</section>
<?php endfor; ?>
...
?>
	...
	function toggleValues(v, n)	{
		...
		x.open("GET", "ViewUpdate.php?table=Contacted&n="+n+"&isread="+
